...
Used Mongodb, an increasing popular document-based, high-performance NoSQL database to store the data that we received from the server. The server is created in nodejs which listens to the POST request published from the mobile application. The server stores the data from the mobile application in JSON format. The server can also execute the GET request to pull data out of mongodb. After setting up the server and database, I worked on the mobile application. The mobile application includes the following components: LocationManager, MediaRecorder, DonutProgress, and HTTPClient. The LocationManager is used to monitor the location of the user and if there is a change in the location, the application will record the noise coming into the mobile. The application also includes asking for user permission to turn on their Global Positioning System (GPS). The application also uses MediaRecorder to perform the recording of the sound coming into the mobile which you perform statistic on by calling the functions in MediaRecorder. In this case, after recording the noise for a second, the application calls the MaxAmplitude() in order to get the max amplitude which is in Pascal unit. However, since decibel is the unit used for sound level, the application needs to perform a mathematical computation of the given Pascal value and convert it to Decibel unit using the following equation.

##### Value_Db = 20 * Log10(abs(Value_Pa))

After computing the sound level into Decibel, we have to collect the time, date, and location data. To communicate with the server, the application uses HttpPost to publish a POST request to the server which is listening for a POST request from the mobile application. Before posting the data, the application needs to create an arraylist of NameValuePair which holds the individual data. Once all the data have been encoded into the arraylist, the HttpClient executes the POST request and as a result, an HttpResponse gets returned which we can use to monitor any error. This process repeats itself until the user stops the mobile application.
